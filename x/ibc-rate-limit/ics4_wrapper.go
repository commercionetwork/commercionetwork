...
// This method retrieves the contract from the middleware's parameters and checks if the sender address has the authorization to transfer or not,
// in which case it returns an error preventing the IBC send from taking place.
...
func (i *ICS4Wrapper) SendPacket(ctx sdk.Context, chanCap *capabilitytypes.Capability, packet exported.PacketI) error {
	...
	if err != nil {
		return sdkerrors.Wrap(err, "Sender doesn't have authorization to transfer")
	}
