...
/*import (
	...
	commercioibctesting "github.com/commercionetwork/commercionetwork/x/ibc-rate-limit/testutil"
...
type MiddlewareTestSuite struct {
	...
	chainA *commercioibctesting.TestChain
	chainB *commercioibctesting.TestChain
...
func NewTransferPath(chainA, chainB *commercioibctesting.TestChain) *ibctesting.Path {
...
func (suite *MiddlewareTestSuite) SetupTest() {
	suite.Setup()
	ibctesting.DefaultTestingAppInit = commercioibctesting.SetupTestingApp
	suite.coordinator = ibctesting.NewCoordinator(suite.T(), 2)
	suite.chainA = &commercioibctesting.TestChain{
		TestChain: suite.coordinator.GetChain(ibctesting.GetChainID(1)),
	}
	// Remove epochs to prevent  minting
	err := suite.chainA.MoveEpochsToTheFuture()
	suite.Require().NoError(err)
	suite.chainB = &commercioibctesting.TestChain{
