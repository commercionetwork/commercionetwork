...
func TestMsgInviteUser_ValidateBasic(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			msg:   missRecipient,
			error: errors.Wrap(sdkErr.ErrInvalidAddress, "Invalid recipient address:  (empty address string is not allowed)"),
		},
		{
			name:  "Missing sender returns error",
			msg:   missSender,
			error: errors.Wrap(sdkErr.ErrInvalidAddress, "Invalid sender address:  (empty address string is not allowed)"),
		},
	}
...
func TestMsgDepositIntoLiquidityPool_ValidateBasic(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			error: errors.Wrap(sdkErr.ErrInvalidAddress, "Invalid depositor address: "),
		},
		{
			name:  "Empty deposit amount returns error",
			msg:   types.MsgDepositIntoLiquidityPool{Depositor: user.String(), Amount: nil},
			error: errors.Wrap(sdkErr.ErrInvalidCoins, "Invalid deposit amount: "),
		},
		{
...
func TestMsgAddTsp_ValidateBasic(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			error: errors.Wrap(sdkErr.ErrInvalidAddress, "Invalid government address: "),
		},
		{
			name:  "Missing tsp returns error",
			msg:   types.MsgAddTsp{Government: government.String(), Tsp: ""},
			error: errors.Wrap(sdkErr.ErrInvalidAddress, "Invalid TSP address: "),
		},
	}
...
func TestMsgBuyMembership_ValidateBasic(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			error: errors.Wrap(sdkErr.ErrInvalidAddress, "Invalid buyer address: "),
		},
		{
			name:  "Missing membership returns error",
			msg:   *types.NewMsgBuyMembership("", testBuyer, testTsp),
			error: errors.Wrap(sdkErr.ErrUnknownRequest, "Invalid membership type: "),
		},
		{
			name:  "Invalid membership returns error",
			msg:   *types.NewMsgBuyMembership("grn", testBuyer, testTsp),
			error: errors.Wrap(sdkErr.ErrUnknownRequest, "Invalid membership type: grn"),
		},
	}
