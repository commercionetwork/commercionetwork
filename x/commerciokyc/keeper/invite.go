...
func (k Keeper) SetInvite(ctx sdk.Context, recipient, sender sdk.AccAddress) error {
	...
	if store.Has(inviteKey) {
		return errorsmod.Wrap(sdkErr.ErrUnknownRequest, fmt.Sprintf("%s has already been invited", recipient))
	}

	// Verify that the user that is inviting has already a membership
	inviterMembership, err := k.GetMembership(ctx, sender)
	if err != nil {
		return errorsmod.Wrap(sdkErr.ErrUnauthorized, fmt.Sprintf("Cannot send an invitation without having a membership: %s", err.Error()))
