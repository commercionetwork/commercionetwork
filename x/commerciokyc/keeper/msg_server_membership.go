...
func (k msgServer) BuyMembership(goCtx context.Context, msg *types.MsgBuyMembership) (*types.MsgBuyMembershipResponse, error) {
	...
	if !found {
		return &types.MsgBuyMembershipResponse{}, errors.Wrap(sdkErr.ErrUnauthorized, "Cannot buy a membership without being invited")
	}

	// Verify invite status
	inviteStatus := types.InviteStatus(invite.Status)
	if inviteStatus == types.InviteStatusInvalid {
		return &types.MsgBuyMembershipResponse{}, errors.Wrap(sdkErr.ErrUnauthorized, fmt.Sprintf("invite for account %s has been marked as invalid previously, cannot continue", msg.Buyer))
	}

	msgTsp, _ := sdk.AccAddressFromBech32(msg.Tsp)
	if !k.IsTrustedServiceProvider(ctx, msgTsp) {
		return &types.MsgBuyMembershipResponse{}, errors.Wrap(sdkErr.ErrUnauthorized, "since you are not a tsp you cannot buy membership")
	}

	// Forbidden black membership buying
	if msg.MembershipType == types.MembershipTypeBlack {
		return &types.MsgBuyMembershipResponse{}, errors.Wrap(sdkErr.ErrUnauthorized, "cannot buy black membership")
	}
	...
	if err == nil && buyerMembership.MembershipType == types.MembershipTypeBlack {
		return &types.MsgBuyMembershipResponse{}, errors.Wrap(sdkErr.ErrUnauthorized, "cannot downgrade from Black membership")
	}
...
func (k msgServer) RemoveMembership(goCtx context.Context, msg *types.MsgRemoveMembership) (*types.MsgRemoveMembershipResponse, error) {
	...
	if !govAddr.Equals(msg.GetSigners()[0]) {
		return nil, errors.Wrap(sdkErr.ErrUnknownAddress,
...
func (k msgServer) SetMembership(goCtx context.Context, msg *types.MsgSetMembership) (*types.MsgSetMembershipResponse, error) {
	...
	if !types.IsMembershipTypeValid(msg.NewMembership) {
		return nil, errors.Wrap(sdkErr.ErrUnknownRequest, fmt.Sprintf("invalid membership type: %s", msg.NewMembership))
	}
	subscriber, _ := sdk.AccAddressFromBech32(msg.Subscriber)

	invite, err := k.governmentInvitesUser(ctx, subscriber)
	if err != nil {
		return nil, errors.Wrap(sdkErr.ErrUnauthorized, fmt.Sprintf("government could not invite user: %s", err.Error()))
	}
	...
	if err != nil {
		return nil, errors.Wrap(sdkErr.ErrUnknownRequest,
