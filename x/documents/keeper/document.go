...
func (keeper Keeper) SaveDocument(ctx sdk.Context, document types.Document) error {
	...
	if _, err := uuid.FromString(document.UUID); err != nil {
		return errors.Wrap(sdkErr.ErrInvalidRequest, fmt.Sprintf("invalid document UUID: %s", document.UUID))
	}
	...
	if store.Has(getDocumentStoreKey(document.UUID)) {
		return errors.Wrap(sdkErr.ErrInvalidRequest, fmt.Sprintf("document with uuid %s already present", document.UUID))
	}
