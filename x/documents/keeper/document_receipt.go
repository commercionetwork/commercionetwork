...
func (keeper Keeper) SaveReceipt(ctx sdk.Context, receipt types.DocumentReceipt) error {
	...
	if _, err := uuid.FromString(receipt.UUID); err != nil {
		return errorsmod.Wrap(sdkErr.ErrInvalidRequest, fmt.Sprintf("invalid document receipt UUID: %s", receipt.UUID))
	}
	store := ctx.KVStore(keeper.storeKey)

	// Check for an existing document
	if !store.Has(getDocumentStoreKey(receipt.DocumentUUID)) {
		return errorsmod.Wrap(sdkErr.ErrInvalidRequest, fmt.Sprintf("receipt points to a non-existing document UUID: %s", receipt.DocumentUUID))
	}
	...
	if store.Has(receiptStoreKey) {
		return errorsmod.Wrap(sdkErr.ErrInvalidRequest, fmt.Sprintf("receipt for document with UUID %s already present: %s", receipt.DocumentUUID, receipt.UUID))
	}
	...
	if !store.Has(getReceivedDocumentsIdsUUIDStoreKey(senderAccAdrr, receipt.DocumentUUID)) {
		return errorsmod.Wrap(sdkErr.ErrInvalidRequest, fmt.Sprintf("sender for document receipt with address %s not among the recipients of the document: %s", receipt.Sender, receipt.UUID))
	}
	...
	if store.Has(sentReceiptsIdsStoreKey) {
		return errorsmod.Wrap(sdkErr.ErrInvalidRequest, fmt.Sprintf("sent receipt for document with UUID %s already present: %s", receipt.DocumentUUID, receipt.UUID))
	}
