...
import (
	...
	authKeeper "github.com/cosmos/cosmos-sdk/x/auth"
...
type SignatureVerificationGasConsumer = func(meter sdk.GasMeter, sig []byte, pubkey crypto.PubKey, params authKeeper.Params) sdk.Result

// NewAnteHandler returns an AnteHandler that checks and increments sequence
// numbers, checks signatures & account numbers, and deducts fees from the first
// signer.
// TODO: implement custom fee checking (see comnet_min_fee_test.go)
func NewAnteHandler(ak authKeeper.AccountKeeper, supplyKeeper types.SupplyKeeper, priceKeeper pricefeed.Keeper, sigGasConsumer SignatureVerificationGasConsumer, stableCreditsDemon string) sdk.AnteHandler {
	return func(
		...
		_, _ = priceKeeper, stableCreditsDemon
		...
		if res := comnetMinFeesChecker(stdTx, ctx, priceKeeper, stableCreditsDemon); !res.IsOK() {
			return newCtx, res, true
		}
		...
		signerAccs := make([]authKeeper.Account, len(signerAddrs))
...
// a transaction.
func GetSignerAcc(ctx sdk.Context, ak authKeeper.AccountKeeper, addr sdk.AccAddress) (authKeeper.Account, sdk.Result) {
...
func ValidateSigCount(stdTx types.StdTx, params types.Params) sdk.Result {
	...
	sigCount := 0
	for i := 0; i < len(stdSigs); i++ {
		sigCount += authKeeper.CountSubKeys(stdSigs[i].PubKey)
...
// a pubkey, set it.
func processSig(
	ctx sdk.Context, acc authKeeper.Account, sig types.StdSignature, signBytes []byte, simulate bool, params types.Params,
	sigGasConsumer SignatureVerificationGasConsumer,
) (updatedAcc authKeeper.Account, res sdk.Result) {
...
func consumeSimSigGas(gasmeter sdk.GasMeter, pubkey crypto.PubKey, sig types.StdSignature, params types.Params) {
	...
	sigBz := authKeeper.ModuleCdc.MustMarshalBinaryLengthPrefixed(simSig)
...
// has not been set.
func ProcessPubKey(acc authKeeper.Account, sig types.StdSignature, simulate bool) (crypto.PubKey, sdk.Result) {
...
func DeductFees(supplyKeeper types.SupplyKeeper, ctx sdk.Context, acc authKeeper.Account, fees sdk.Coins) sdk.Result {
...
// and an account.
func GetSignBytes(chainID string, stdTx types.StdTx, acc authKeeper.Account, genesis bool) []byte {
	var accNum uint64
	if !genesis {
