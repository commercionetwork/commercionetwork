...
func (msg *MsgIncrementBlockRewardsPool) ValidateBasic() error {
	...
	if err != nil {
		return errorsmod.Wrapf(sdkerrors.ErrInvalidAddress, "invalid funder address (%s)", err)
	}

	// msg.Amount.IsAnyNegative() is redundant: sdk.Coins are checked to be positive
	if msg.Amount.IsZero() || msg.Amount.IsAnyNegative() {
		return errorsmod.Wrap(sdkerrors.ErrUnknownRequest, "You can't transfer a null or negative amount")
	}
...
func (msg *MsgSetParams) ValidateBasic() error {
	...
	if err != nil {
		return errorsmod.Wrapf(sdkerrors.ErrInvalidAddress, "invalid government address format: %s", err)
	}

	params := NewParams(msg.DistrEpochIdentifier, msg.EarnRate)

	if err := params.Validate(); err != nil {
		return errorsmod.Wrap(sdkerrors.ErrInvalidType, fmt.Sprintf("invalid params: %s", err))
	}
