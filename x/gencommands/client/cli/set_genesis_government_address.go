...
func SetGenesisGovernmentAddressCmd(ctx *server.Context, cdc *codec.Codec,
	...
	cmd := &cobra.Command{
		...
		RunE: func(cm *cobra.Command, args []string) error {
			return setGenesisGovernmentAddressCmdFunc(cm, args, ctx, cdc)
		},
	}
...
func setGenesisGovernmentAddressCmdFunc(_ *cobra.Command, args []string, ctx *server.Context, cdc *codec.Codec) error {
	config := ctx.Config
