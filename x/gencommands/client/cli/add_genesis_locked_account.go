...
func AddGenesisLockedAccountCmd(ctx *server.Context, cdc *codec.Codec,
	...
	cmd := &cobra.Command{
		...
		RunE: func(cm *cobra.Command, args []string) error {
			return addGenesisLockedAccountCmdFunc(cm, args, ctx, cdc)
		},
	}
...
func addGenesisLockedAccountCmdFunc(_ *cobra.Command, args []string, ctx *server.Context, cdc *codec.Codec) error {
	config := ctx.Config
