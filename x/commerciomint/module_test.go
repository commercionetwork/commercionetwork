...
func TestAppModuleBasic(t *testing.T) {
	...
	require.Equal(t, `{"positions":[],"pool_amount":[],"credits_denom":"uccc","collateral_rate":"2.000000000000000000"}`, string(amb.DefaultGenesis()))

	require.Error(t, amb.ValidateGenesis(json.RawMessage(`{}`)))
	require.NoError(t, amb.ValidateGenesis(json.RawMessage(`{"positions":[],"pool_amount":[],"credits_denom":"uccc","collateral_rate":"2.000000000000000000"}`)))
	require.Error(t, amb.ValidateGenesis(json.RawMessage(``)))
