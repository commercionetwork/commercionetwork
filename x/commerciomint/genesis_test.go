...
func TestInitGenesis(t *testing.T) {
	...
	require.Equal(t, commerciomint.GenesisState{Positions: []types.Position{}, LiquidityPoolAmount: sdk.NewCoins(), CollateralRate: sdk.NewDec(1), FreezePeriod: commerciomint.DefaultFreezePeriod}, got)
	commerciomint.InitGenesis(ctx, k, sk, got)
	export := commerciomint.ExportGenesis(ctx, k)
	require.Equal(t, commerciomint.GenesisState{Positions: []types.Position(nil), LiquidityPoolAmount: sdk.Coins(nil), CollateralRate: sdk.NewDec(1), FreezePeriod: commerciomint.DefaultFreezePeriod}, export)
